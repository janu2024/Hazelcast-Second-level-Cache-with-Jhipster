...
public class CacheConfiguration {
    ...
    public HazelcastInstance hazelcastInstance(JHipsterProperties jHipsterProperties) {
        ...
        HazelcastInstance hazelCastInstance = Hazelcast.getHazelcastInstanceByName("test2");
        if (hazelCastInstance != null) {
            log.debug("Hazelcast already initialized");
            return hazelCastInstance;
        }
        Config config = new Config();
        config.setInstanceName("test2")
        		.addMapConfig(new MapConfig().setName("cache")
				.setMaxSizeConfig(new MaxSizeConfig(200, MaxSizeConfig.MaxSizePolicy.FREE_HEAP_SIZE))
				.setEvictionPolicy(EvictionPolicy.LRU).setTimeToLiveSeconds(0))
        		.addMapConfig(new MapConfig().setName("cache1")
        		.setMaxSizeConfig(new MaxSizeConfig(200,MaxSizeConfig.MaxSizePolicy.FREE_HEAP_SIZE))
        		.setEvictionPolicy(EvictionPolicy.LRU).setTimeToLiveSeconds(1000));
        ...
        config.getMapConfigs().put("com.demo.app.domain.*", initializeDomainMapConfig(jHipsterProperties));
